...
from src.core.dto.user.role import UserRoleDTO, UserUserRoleDTO, UserUserRoleUpdateDTO
...
class TaskCreateUseCase:
    ...
    async def __call__(
        self, *, updating_user: User, admin_user: User, obj: UserRoleDTO
    ) -> Result:
        ...
        if not updating_user:
            ...
            )  # Может есть возможность объеденить?...

        user = UserUserRoleUpdateDTO(
            username=updating_user.username, application_role=obj.name
        )
