...
class UserSubscriptionUpdateUseCase:
    def __init__(
            self, user_sub_repo: IUserSubscriptionRepository, 
    ...
    async def __call__(self, user: User, obj: UserSubscriptionUpdateDTO) -> Result:
        ...
        if user.is_premium and not obj.subscription_id == user.subscription.id:
            ...
            user_sub = await self.user_sub_repo.get(user_id=user.username, sub_id=user.subscription.id) #Мб тут Юзера вернуть?... Лишний запрос мб?
