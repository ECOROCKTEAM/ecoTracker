...
from tests.const import UsersT
...
async def test_user(pool: async_sessionmaker[AsyncSession]) -> User:
    ...
    user_id = UsersT.USER__ACTIVE.value
    async with pool() as s:
        user = await s.get(UserModel, {"id": user_id})
    if user is None:
        raise RuntimeError("Fixture not ready")
    return User(
        username=user.username, password=user.password, active=True, id=user.id, language=lang, subscription=""  # type: ignore
    )
